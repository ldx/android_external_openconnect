...
static int load_bio_certificate(struct openconnect_info *vpninfo,
								BIO *cert_bio, BIO *key_bio)
{
	EVP_PKEY *evp = PEM_read_bio_PrivateKey(key_bio, NULL, NULL, NULL);
	BIO_free(key_bio);

	vpninfo->cert_x509 = PEM_read_bio_X509(cert_bio, NULL, NULL, NULL);
	BIO_free(cert_bio);
...
static int load_bio_cacert(struct openconnect_info *vpninfo, BIO *bio)
{
	int i;
	X509_STORE *cert_ctx;
...
#ifdef ANDROID
	if (vpninfo->cert_type == CERT_TYPE_KEYSTORE ||
		vpninfo->cert_type == CERT_TYPE_UNKNOWN) {
		BIO *b = BIO_from_keystore(vpninfo->cert);
		if (b)
			return load_bio_certificate(vpninfo, b,
				BIO_from_keystore(vpninfo->sslkey));
	}
#endif
...
#ifdef ANDROID
			/*
			 * Try to load CA certificate from keystore, but just fall
			 * through if this fails.
			 */
			BIO *bio = BIO_from_keystore(vpninfo->cafile);
			if (bio) {
				err = load_bio_cacert(vpninfo, bio);
				if (err) {
					close(ssl_sock);
					return err;
				}
